...
// rotate eye position in WCS
	...
	//Create a matrix and rotate over; x, y or z axis depending on what's passed
	Matrix matrix;
	matrix.rotateMatrix(rx,ry,rz,angle);

	//Create a vector of the curren x, y and z values
	GLfloat vector[4];
	vector[0] = eye.x;
	vector[1] = eye.y;
	vector[2] = eye.z;
	...
	//Multiply the rotated matrix by the vector
	matrix.multiplyVector(vector);

	//update the eye's x, y and z to the new vector
	eye.x = vector[0];
	eye.y = vector[1];
	eye.z = vector[2];
