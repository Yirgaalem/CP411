...
void Bezier::computeBezPt(GLfloat u, GLint nCtrlPts, Point ctrlPts[], GLint C[], Point *bezPt){
  // your code *BezierCurve*
	 double b[nCtrlPts], x = 0, y = 0, z = 0, r = 0;
	 int n = nCtrlPts - 1;

	if(u < .00001){
		bezPt->x = ctrlPts[0].x; bezPt->y = ctrlPts[0].y; bezPt->z = ctrlPts[0].z;
	}
	else if(u > .99999){
		bezPt->x = ctrlPts[n].x; bezPt->y = ctrlPts[n].y; bezPt->z = ctrlPts[n].z;
	}
	else{
		r = u/(1.0 - u);
		b[0] = pow(1.0-u,n);
...
// your code
	...
	double b[n], x=0, y=0, z=0, r,x2=0,y2=0,z2=0;
	if (u < .00001){
		...
		bezTan->x = n*x; bezTan->y = n*y; bezTan->z = n*z;
		r = u / (1.0 - u);
		b[0] = pow(1.0 - u, n);
		for (int i = 1; i <= n; ++i)
			b[i] = b[i - 1] * (n - i + 1) / i * r;
		...
		for (int i = 0; i <= n; ++i) {
			x2 = x2 + b[i] * ctrlPts[i].x;
			y2 = y2 + b[i] * ctrlPts[i].y;
			z2 = z2 + b[i] * ctrlPts[i].z;
		}
