...
void init(void) {
	...
	glutCreateWindow("SimpleDraw Nahor Yirgaalem");
...
void mouseActionFcn(GLint button, GLint action, GLint xMouse, GLint yMouse) {
	...
	if (oprMode == 0 && isInMenu == 0) {
		if (button == GLUT_LEFT_BUTTON) {
			...
			if (action == GLUT_DOWN) {
				...
				SHAPE *newObj = (SHAPE *) malloc(sizeof(SHAPE));
				*newObj = tempObj;
   ...
   // edit move mode
		...
		if (button == GLUT_LEFT_BUTTON) {
			...
			} else if (action == GLUT_UP) { //RIGHT HERE
...
void mouseMotionFcn(GLint xMouse, GLint yMouse) {
	...
	if(isInDraw == 1){ //if in drawing mode
		tempObj.x2 = xMouse;
		tempObj.y2 = yMouse;
	}

	else if(isInMove == 1){ //if in edit mode
		if(selectNode != NULL){
...
int main(int argc, char** argv) {
	...
	addMenu();
	init();
	glutDisplayFunc(drawObjectList); ///IMPORTANT
